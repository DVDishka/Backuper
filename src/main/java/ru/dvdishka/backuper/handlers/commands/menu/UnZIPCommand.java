...
public class UnZIPCommand extends ConfirmableCommand {
    ...
    public boolean check() {
        ...
        if (!sender.hasPermission(Permission.UNZIP.getPermission(storage))) {
    ...
    @Override
    public void run() {
        Backuper.getInstance().getTaskManager().startTaskAsync(backup.getUnZipTask(), sender, List.of(Permission.UNZIP.getPermission(storage)));
    }
}
